...
function goods_shop_fuel($id){
    ...
    $map['status']=1;
    return M("shop_goods_fuel")->where($map)->select();
}
//季度区间时间戳
function  season_section_time($i,$data=''){
    $result = array();
    for ($season=$i?$i:1; $season <= 4; $season++) { 
        $tmp =  array();
        $start = mktime(0, 0, 0,$season*3-3+1,1,date('Y'));
        $end = mktime(23,59,59,$season*3,date('t',mktime(0, 0 , 0,$season*3,1,date("Y"))),date('Y'));
        $tmp = array($start,$end);
        if($data=='date'){
           $tmp = array_map("format_date",$tmp);
        }
        $result[] = array('season'=>$season,'time'=>$tmp);
        if($i) break;
    }
    return $result;
}
...
function cityList(){
    $result = M('shop_city')->where('status=1')->field('id,city,area,sends')->select();
    return $result;
}
