...
<body>
    ...
    <tbody>
		<?php if(!empty($_list)): if(is_array($_list)): $i = 0; $__LIST__ = $_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
            <td><input class="ids" type="checkbox" name="id[]" value="<?php echo ($vo["uid"]); ?>" /><input type="hidden" name="id" value=""></td>
			...
			<td><?php if(($vo["status"]) == "1"): ?><a href="<?php echo U('User/changeStatus?method=forbidUser&id='.$vo['uid']);?>" class="ajax-get">禁用</a>
				...
				<a href="<?php echo U('User/edit',array('uid'=>$vo['uid']));?>">修改</a>
				<a href="<?php echo U('User/changeStatus?method=deleteuser&id='.$vo['uid']);?>" class="ajax-get">删除</a>
                </td>
